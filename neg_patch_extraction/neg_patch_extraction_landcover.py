...
def read_and_extract(img, img_rast, x, y, landcover, img_name, out_path):
    ######################################################
    ...
    try:
        ...
        cv2.imwrite(out_path + str(landcover) + '/' + img_name + ".png", sliced_img)
...
def save_patches(slice_no, slice_length, pickled_files_path, dg_path, landuse_path, output_path, buff_radius):
    print("Reading kenya's land cover raster file", flush=True)
    dlc = gr.from_file(landuse_path)
        ...
        for fname in df.file_name.unique():
            ...
            if len(iter_list) !=0:
                ...
                try:
                    ...
                    print("Geometry points found!")
                except:
                    print("Error in finding geom points. Skipping iteration!")
                    continue
            ...
            nobuild_list = generate_random_spatialpoints(2000, nobuild_poly)
            ...
            for p in nobuild_list:
                ...
                try:
                    x, y = p
                    landcover = dlc.map_pixel(x,y)
                    print("found associated land cover type")
                    if str(landcover) == "--": #Happens when no landcover can be mapped
                        landcover = 'notype'
                    print("{} - Obtained (x,y) coordinates: {}".format(locs_counter, p), flush=True)
                    img_name = foldername + "_z_" + filename.split('.')[0] + "_z_" + str(x) + "_z_" + str(y) + "_z_" + str(landcover) + "_z_" + "nobuild"
                    print("Sending locs for a point:{} for patch extraction".format(p), flush=True)
                    patch = read_and_extract(sat_img, sat_img_rast, x, y, landcover, img_name, output_path)
                except:
...
output_path = "/mnt/nfs/eguide/projects/bldgpop/patches/no_buildings_landcover/"
landuse_path = "/home/zshah/DG_struct_locs/landuse_data/geokenya.tif"

print('{} : Starting'.format(time.strftime('%m/%d/%Y %H:%M:%S',time.localtime())))
save_patches(slice_no, slice_length, pickled_files_path, dg_path, landuse_path, output_path, dist_val)
